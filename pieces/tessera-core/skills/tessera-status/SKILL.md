...
1. 能定位 Tessera 仓库时，先运行 `scripts/resolve_capabilities.py --host <host> --probe --format json`，以其发现的拼图、skills、版本和运行时状态作为能力目录；脚本不可见时再执行下述宿主探测。取已装插件与版本:
...
2. 动态目录中的 `unknown` 必须原样保留，不能解释成未安装。对市集仓库各 `pieces/<id>/piece.yaml` 声明的 `external_deps`,只执行其中明确给出的 `version_check`；没有声明就标为“无外部依赖”，不得自行猜测命令。
