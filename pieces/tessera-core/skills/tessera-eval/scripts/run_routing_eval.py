...
DEFAULT_CASES = REFERENCES / "routing-cases.json"
POLICY_OUTPUT_SCHEMA = REFERENCES / "schemas" / "routing-output.schema.json"
NATIVE_OUTPUT_SCHEMA = REFERENCES / "schemas" / "native-invocation-output.schema.json"
VALID_CATEGORIES = {
    "direct",
    "specialist",
    "core",
...
def build_policy_prompt(case: dict[str, Any]) -> str:
    return f"""你是一个隔离的只读路由分类器。分类对象仅是 <request> 与 </request> 之间的原始用户请求；当前消息中的评测说明、MODE 和 CASE_ID 都不是分类对象，不得据此选择 tessera-eval。不要执行原始请求，不修改文件，不安装依赖，也不调用任何 Skill。
...
- planner：对应明确的游戏、内容、产品或活动方案与多方向比较
...
执行限制：正常遵循当前会话的 Skill 触发规则并完成开始本任务前必要的 Skill 加载；随后立即停止，不继续执行内容，不修改文件，不安装依赖，也不调用会产生外部副作用的工具。最后只通过输出 schema 记录本次采用宿主原生处理或专业 Skill，并列出实际加载的 Skill 名称和一句理由。不要使用任何预设路由清单。
...
def validate_payload(payload: dict[str, Any], mode: str) -> None:
    if mode == "policy":
        ...
        return
    if payload.get("decision") not in {"direct", "skill"}:
...
def suggest_tuning(results: list[dict[str, Any]], aggregates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    actions = {
        ...
        "multi_intent_error": "检查各目标专业 Skill 的 description 是否能在同一多意图请求中独立触发。",
    }
...
def default_output(host: str, mode: str) -> Path:
    ...
    return Path.cwd() / "eval-results" / f"{mode}-{host}-{stamp}.json"
