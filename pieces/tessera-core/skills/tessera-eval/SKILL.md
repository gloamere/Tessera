...
若能定位仓库 `scripts/usage_events.py`，进入时尝试运行 `start --host <host> --skill tessera-eval --project <cwd>`；只有返回事件 id 时，结束前运行对应 `finish --outcome completed|failed`。记录失败不得影响评测，也不得混入 `eval-results`。
...
个人场景使用 `tests/personal-routing-cases.yaml`。每个 native 案例必须显式声明 `expected_skills`；直接任务使用空数组，新增能力准入映射为 `piece-router`。

## 执行

```powershell
# 政策分类；兼容旧调用
python scripts/run_routing_eval.py --host codex --mode policy

# 原生调用验证
python scripts/run_routing_eval.py --host codex --mode native

# 个人场景
python scripts/run_routing_eval.py --host codex --mode native --cases tests/personal-routing-cases.yaml

# 确认随机失败是否稳定复现
python scripts/run_routing_eval.py --host codex --mode native --case <id> --repeat 3

# 只输出边界优化建议，不修改 Skill
python scripts/run_routing_eval.py --host codex --mode native --case <id> --repeat 3 --suggest-tuning
```
...
- 多意图错误：澄清 `piece-router` 对多个独立交付物和高影响协调的边界。
